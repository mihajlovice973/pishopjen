...
-- ХОТИМ 120x80, НО железо OpenComputers имеет предел:
-- максимумы по tier: 50x16 / 80x25 / 160x50 (выше 50 по высоте невозможно).
-- Поэтому автоматически снижаем до максимально поддерживаемой.
...
local accent = 0x00E5C9          -- цвет логотипа и рамки

-- Отступы для текста, чтобы он не залезал на нижнюю рамку
local STATUS_Y = h - 4
local STATUS_Y2 = h - 3
local PROMPT_Y = h - 2
local SUBPROMPT_Y = h - 1
...
-- Функция отрисовки красивой рамки
...
local function center(height, text, color)
    gpu.fill(2, height, w - 2, 1, " ") -- не затираем рамку
...
local function drawInfo(type)
    local line = 14
    if type == "full" then
        -- очищаем только внутри рамки, до статус-строк
        gpu.fill(2, 2, w - 2, STATUS_Y - 2, " ")
    end
    for i, ore in pairs(ore_list) do
        local print_row = line + i
        if print_row > STATUS_Y - 1 then break end -- список не наезжает на статус
        if type == "full" then
        ...
        if type == "full" or type == "ingots" then
            gpu.fill(100, print_row, 15, 1, " ")
            gpu.setForeground(0xFF00FF)
            gpu.set(100, print_row, formatNumber(ore.size or 0))
        end
        line = line + 1
    end
end
...
local function drawLogo(x, y, color)
    -- центрирование логотипа по ширине экрана
    local logoWidth = 74
    local startX = math.floor((w - logoWidth) / 2)
    local dragon_x = startX
    local exchanger_x = startX
...
local function handleEvent(eventName, ...)
    ...
    -- Отслеживаем нажатие Ctrl+C (код 3)
    if eventName == "key_down" then
        local char = args[2]
        local player = args[4]
        if char == 3 or char == "\x03" then
            lastCtrlCUser = player
        end
    elseif eventName == "interrupted" then
        -- Админ - выходим в shell, обычный игрок - игнорируем
