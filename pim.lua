...
-- ================== БЛОК ОБНОВЛЕНИЯ (By KaRMa__) ==================
local UPD_CONFIG_PATH = "/admin_config.cfg"
local UPD_HOME = "/home"

local function updLoadConfig()
    if fs.exists(UPD_CONFIG_PATH) then
        local f = io.open(UPD_CONFIG_PATH, "r")
        local ok, cfg = pcall(serialization.unserialize, f:read("*a"))
        f:close()
        if ok and type(cfg) == "table" and type(cfg.admins) == "table" then return cfg end
    end
    local cfg = { admins = { "KaRMa__" } }
    local f = io.open(UPD_CONFIG_PATH, "w")
    f:write(serialization.serialize(cfg))
    f:close()
    return cfg
end
local updConfig = updLoadConfig()

local function isAdmin(name)
    for _, a in ipairs(updConfig.admins) do
...
local function setLine(x, y, w, text, color)
    ...
    if len > w then
        ...
        for c in t:gmatch("[%z\1-\127\192-\255][\128-\191]*") do
            cnt = cnt + 1
            if cnt <= w then out = out .. c else break end
        end
        t, len = out, w
    end
    gpu.set(x, y, t .. string.rep(" ", w - len))
end

local FRAME = { tl="╔", tr="╗", bl="╚", br="╝", h="═", v="║" }

local function drawFrame(x, y, w, h)
    gpu.set(x, y, FRAME.tl .. string.rep(FRAME.h, w - 2) .. FRAME.tr)
    for i = 1, h - 2 do
        gpu.set(x, y + i, FRAME.v .. string.rep(" ", w - 2) .. FRAME.v)
    end
    gpu.set(x, y + h - 1, FRAME.bl .. string.rep(FRAME.h, w - 2) .. FRAME.br)
end

local function drawUpdateInterface()
    gpu.fill(1, 1, 80, 25, " ")
    local fw, fh = 60, 15
    ...
    gpu.set(fx + math.floor((fw - ulen(title)) / 2), fy - 1, title)
    gpu.setForeground(0x00FF00)
    drawFrame(fx, fy, fw, fh)
    gpu.setForeground(0xFFFFFF)
    gpu.set(1, 25, "By KaRMa__")
    return fx, fy, fw, fh
end

local function drawBar(x, y, w, p)
    local inner = w - 2
...
local function showStatus(fx, fy, fw, fh, l1, l2, l3, color)
    local cy = fy + math.floor(fh / 2) - 1
    setLine(fx + 2, cy,     fw - 4, l1 or "", color)
    setLine(fx + 2, cy + 1, fw - 4, l2 or "", color)
    setLine(fx + 2, cy + 2, fw - 4, l3 or "", color)
end
...
local function updDownloadFile(url, path)
...
local function updateMode()
    local fx, fy, fw, fh = drawUpdateInterface()
    drawBar(fx, fy + fh + 1, fw, 0)
    local total = #UPD_FILES
    local done = 0
    for i, file in ipairs(UPD_FILES) do
        fx, fy, fw, fh = drawUpdateInterface()
        drawBar(fx, fy + fh + 1, fw, done / total)
        showStatus(fx, fy, fw, fh, "Идёт скачивание:", fs.name(file.path),
                   "Скачано: " .. done .. "/" .. total, 0xFFFFFF)
        local success = false
        for attempt = 1, 2 do
            if updDownloadFile(file.url, file.path) then success = true break end
            os.sleep(1)
        end
        fx, fy, fw, fh = drawUpdateInterface()
        if success then
            done = done + 1
            showStatus(fx, fy, fw, fh, "Готово:", fs.name(file.path),
                       "Скачано: " .. done .. "/" .. total, 0x00FF00)
        else
            showStatus(fx, fy, fw, fh, "Ошибка:", fs.name(file.path),
                       "Скачано: " .. done .. "/" .. total, 0xFF0000)
        end
        drawBar(fx, fy + fh + 1, fw, done / total)
        os.sleep(0.3)
    end
    if done == total then
        setupAutorun()
        for i = 5, 1, -1 do
            showStatus(fx, fy, fw, fh, "Обновление завершено!", "",
                       "Перезагрузка через " .. i .. " сек...", 0x00FF00)
            os.sleep(1)
        end
        computer.shutdown(true)
        return true
    else
        showStatus(fx, fy, fw, fh, "Ошибка при обновлении!", "",
                   "Перезагрузка не выполнена.", 0xFF0000)
        os.sleep(3)
        return false
    end
end
...
local function main()
    ...
    while true do
        ...
        if e == "key_down" and ev[3] == 7 then -- Ctrl+G
            local playerName = ev[5] or ""
            if isAdmin(playerName) then
                updateMode()
                return
            else
                showTempMessage("Доступ запрещён! Только для администрации.", 2)
            end
            goto continue
        end
